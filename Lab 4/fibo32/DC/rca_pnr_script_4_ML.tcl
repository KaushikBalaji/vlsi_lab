...
#create a new design library for your design with some name <FIBO_LIB>
create_lib -ref_lib $PDK_PATH/tsl18fs120_scl.ndm FIBO_LIB

#read the mapped netlist and pass the library 
read_verilog {./inputs/fibo.mapped.v} -library FIBO_LIB
...
write_verilog ./results/fibo.routed.v
write_sdc -output ./results/fibo.routed.sdc
write_parasitics -format spef -output ./results/fibo.spef
write_def ./results/fibo.def
# man write_gds 

################################## STA #################################
## check for setup voilation
report_timing -delay_type max > ./../../fibo_setup_report.rpt

## check for hold voilation
report_timing -delay_type min > ./../../fibo_hold_report.rpt
